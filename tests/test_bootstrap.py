...
@pytest.mark.trio
async def test_from_config_file(loop):
    """Test with configuration file."""
    with patch('homeassistant.bootstrap.conf_util.process_ha_config_upgrade', Mock()), \
        patch('homeassistant.util.location.detect_location_info', Mock(return_value=None)), \
...
@pytest.mark.trio
async def test_home_assistant_core_config_validation(loop):
    """Test if we pass in wrong information for HA conf."""
    # Extensive HA conf validation testing is done
    with patch('homeassistant.bootstrap.async_enable_logging', Mock()), \
        patch('homeassistant.bootstrap.async_register_signal_handling', Mock()):
        hass = core.HomeAssistant(loop=loop)
